...
def calc_global_features(cfg: DictConfig) -> None:
    ...
    histograms_dir = "channel_histograms"
    ...
    for channel in range(cfg.encoder.input_features if render_histograms else 1):
        ...
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            ...
            # Update histogram for channel values
            channel_0_values = flat_features[channel, :]  # Get channel 0
         ...
         # Plotting the histogram for the current channel
        ...
        plt.title(f'Logarithmic Histogram of Channel {channel} Values')
        plt.xlabel('Value (log scale)')
        plt.ylabel('Count')
        
        # Save each histogram in the dedicated folder
        plt.savefig(f'{histograms_dir}/log_channel_{channel}_histogram.png')
        plt.close()
